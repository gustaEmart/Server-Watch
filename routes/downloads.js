...
export function createDownloadHandler({ downloads, getSession, authorizeProbe, sendJson, notFound }) {
  return async function serveDownload(req, res) {
    ...
    if (typeof download.render === "function") {
      try {
        const body = await download.render(req, { session, hasProbeToken, hasAdminSession });
        res.writeHead(200, {
        ...
        sendJson(res, error.statusCode || 400, { error: error.message || "Nao foi possivel gerar o download." });
      }
      return;
    }
