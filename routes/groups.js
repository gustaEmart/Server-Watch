export function createGroupsHandler({
  ...
  return async function handleGroups(req, res, { parts, session }) {
    if (req.method === "GET" && parts.length === 2) {
      return sendJson(res, 200, visibleGroups(session.user).map(publicGroup));
    }
    ...
    if (req.method === "GET" && parts.length === 3) {
      if (!visibleGroups(session.user).some((item) => item.id === group.id)) return notFound(res);
