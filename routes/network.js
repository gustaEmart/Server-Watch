export function createNetworkHandler({
  ...
  return async function handleNetwork(req, res, { parts }) {
    if (parts[2] === "devices") {
      if (req.method === "GET" && parts.length === 3) {
        return sendJson(res, 200, getDevices().filter((device) => !device.deletedAt).map(publicDevice));
      }
    ...
    if (parts[2] === "links") {
      if (req.method === "GET" && parts.length === 3) {
        return sendJson(res, 200, getLinks().filter((link) => !link.deletedAt).map(publicLink));
      }
