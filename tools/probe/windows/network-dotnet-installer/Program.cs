...
namespace ServerWatchNetworkProbeSetup;

internal static class Program
{
    private const string TaskName = "ServerWatch Network Probe";
    private const string NodeRuntimeDownloadPath = "/downloads/probe/node-runtime-windows-x64";
    private static readonly Lazy<string> EmbeddedCollectorVersion = new(ReadEmbeddedCollectorVersion);
    private static string CollectorVersion => EmbeddedCollectorVersion.Value;
    private static readonly string InstallDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),
        "ServerWatchNetworkProbe"
    );
    private static readonly string ConfigPath = Path.Combine(InstallDir, "config.json");
    private static readonly string SnmpDir = Path.Combine(InstallDir, "snmp");
    ...
    private static int RunSilentRepair()
    {
        try
        {
            ...
            var config = new ProbeConfig(
                ...
                node["intervalSeconds"]?.GetValue<int?>() ?? 60,
                node["timeoutMs"]?.GetValue<int?>() ?? 3000
            );
            ...
            try
            {
                ...
                Console.WriteLine("Copiando arquivos do network collector...");
                InstallerForm.WriteCollectorFiles();
                ...
                Console.WriteLine("Iniciando Network Probe...");
    ...
    private static string ReadEmbeddedCollectorVersion()
    {
        var assembly = Assembly.GetExecutingAssembly();
        using var stream = assembly.GetManifestResourceStream("network-collector.js");
        if (stream is null)
        {
            return "unknown";
        }
    ...
    private sealed class InstallerForm : Form
    {
        ...
        {
            Text = "ServerWatch Network Probe";
            ...
            var title = new Label
            {
                Text = "Instalar probe de rede (SNMP)",
                Font = new Font("Segoe UI", 14, FontStyle.Bold),
                ForeColor = System.Drawing.Color.White,
                BackColor = Color("#0b2545"),
                TextAlign = ContentAlignment.MiddleRight,
                Location = new Point(198, 24),
            ...
            {
                Text = "Coleta CPU/RAM/interfaces via SNMP e envia pro ServerWatch central.",
                ForeColor = System.Drawing.Color.White,
                BackColor = Color("#0b2545"),
                TextAlign = ContentAlignment.MiddleRight,
                Location = new Point(198, 55),
            ...
            ConfigureNumber(intervalSeconds, 22, 230, 120, 10, 3600, 60);

            AddLabel("Timeout em ms", 172, 208, 140);
            ConfigureNumber(timeoutMs, 172, 230, 120, 500, 60000, 3000);
            ...
            status.Text = $"O network probe sera instalado em {InstallDir}.";
        ...
        private void LoadExistingConfig()
        {
            if (!File.Exists(ConfigPath))
            {
                probeId.Text = $"{Environment.MachineName}-rede";
                probeName.Text = probeId.Text;
                return;
            }
            ...
            try
            {
                ...
                intervalSeconds.Value = Math.Clamp(json["intervalSeconds"]?.GetValue<int>() ?? 60, 10, 3600);
                timeoutMs.Value = Math.Clamp(json["timeoutMs"]?.GetValue<int>() ?? 3000, 500, 60000);
            }
            catch
            {
        ...
        private void Install()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    SetProgress(20, "Copiando arquivos do network collector...");
                    WriteCollectorFiles();
                    ...
                    RegisterTask();
                    SetProgress(82, "Iniciando Network Probe...");
                    RunTask();
                ...
                SetProgress(100, "Instalacao concluida. O network probe ja foi registrado no ServerWatch.");
                MessageBox.Show(
                    "ServerWatch Network Probe instalado, iniciado e registrado com sucesso.",
                    "ServerWatch Network Probe",
                    MessageBoxButtons.OK,
        ...
        {
            var url = $"{config.ServerUrl.TrimEnd('/')}/api/network-probe/validate?probeId={Uri.EscapeDataString(config.ProbeId)}";
        ...
        {
            var url =
                $"{config.ServerUrl.TrimEnd('/')}/api/network-probe/targets" +
                $"?probeId={Uri.EscapeDataString(config.ProbeId)}" +
                $"&name={Uri.EscapeDataString(config.Name)}" +
                $"&version={Uri.EscapeDataString(CollectorVersion)}" +
        ...
        internal static void WriteCollectorFiles()
        {
            Directory.CreateDirectory(SnmpDir);
        ...
        {
            var collectorPath = Path.Combine(InstallDir, "network-collector.js");
            var taskArguments = $"\"{collectorPath}\" --config \"{ConfigPath}\"";
            var xmlPath = Path.Combine(Path.GetTempPath(), $"serverwatch-network-probe-task-{Guid.NewGuid():N}.xml");
            var taskXml =
                "<?xml version=\"1.0\" encoding=\"UTF-16\"?>\n" +
                "<Task version=\"1.2\" xmlns=\"http://schemas.microsoft.com/windows/2004/02/mit/task\">\n" +
                "  <RegistrationInfo>\n" +
                "    <Description>ServerWatch Network Probe</Description>\n" +
                "  </RegistrationInfo>\n" +
                "  <Triggers>\n" +
                "    <BootTrigger>\n" +
            ...
            foreach (var process in Process.GetProcessesByName("node"))
            {
                try
                {
                    if (process.MainModule?.FileName is string processPath &&
                        string.Equals(processPath, NodePath, StringComparison.OrdinalIgnoreCase))
                    {
                        process.Kill(true);
                    }
                }
        ...
        internal static void EnsureNodeRuntime(ProbeConfig config)
        {
            ...
            var tempRoot = Path.Combine(Path.GetTempPath(), $"ServerWatchNetworkProbeNode.{Guid.NewGuid():N}");
        ...
        internal static string? CreateBackup()
        {
            ...
            var backupDir = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),
                $"ServerWatchNetworkProbe.backup.{DateTime.Now:yyyyMMddHHmmss}"
            );
        ...
        {
            if (MessageBox.Show(
                    "Remover o ServerWatch Network Probe deste Windows?",
                    "ServerWatch Network Probe",
            ...
            try
            {
                ...
                SetProgress(100, "Network Probe removido.");
                MessageBox.Show("Network Probe removido.", "ServerWatch Network Probe", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Close();
            }
            catch (Exception error)
            {
